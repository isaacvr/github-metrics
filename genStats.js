...
var contribs             = {};
var commitsPerMonth      = {};
var pullsTimeScaleDay    = {};
var pullsTimeScaleWeek   = {};
var pullsTimeScaleMonth  = {};
var issuesTimeScaleDay   = {};
var issuesTimeScaleWeek  = {};
var issuesTimeScaleMonth = {};
...
function addCommit(commit) {
  ...
  if (commitsPerMonth.hasOwnProperty(mnt) === false) {
    commitsPerMonth[mnt] = [];
  }

  var len1 = commitsPerMonth[mnt].length;
  var fnd = false;

  for (i = 0; i < len1; i += 1) {
    if (commitsPerMonth[mnt][i].id === id) {
      commitsPerMonth[mnt][i].name = user;
      commitsPerMonth[mnt][i].commits += 1;
      fnd = true;
      break;
    }
  }
...
function pullsTimeScaleAdd(pr) {
  ...
  pullsTimeScaleDay[day] = pullsTimeScaleDay[day] || [];
  pullsTimeScaleDay[day].push(pr);

  pullsTimeScaleWeek[week] = pullsTimeScaleWeek[week] || [];
  pullsTimeScaleWeek[week].push(pr);

  pullsTimeScaleMonth[month] = pullsTimeScaleMonth[month] || [];
  pullsTimeScaleMonth[month].push(pr);
...
/// Issues
function issuesTimeScaleAdd(iss) {
  ...
  issuesTimeScaleDay[day] = issuesTimeScaleDay[day] || [];
  issuesTimeScaleDay[day].push(iss);

  issuesTimeScaleWeek[week] = issuesTimeScaleWeek[week] || [];
  issuesTimeScaleWeek[week].push(iss);

  issuesTimeScaleMonth[month] = issuesTimeScaleMonth[month] || [];
  issuesTimeScaleMonth[month].push(iss);
...
function pullsStats() {
  ...
  var file = BASE_DIR + '/pulls_bundle.json';
...
purge(references);
purge(contribs);
purge(commitsPerMonth);
purge(pullsTimeScaleDay);
purge(pullsTimeScaleWeek);
purge(pullsTimeScaleMonth);
purge(issuesTimeScaleDay);
purge(issuesTimeScaleWeek);
purge(issuesTimeScaleMonth);
